  ...
  return db.prepare(`
    INSERT INTO products
      (name, slug, sku, category, price, unit, min_qty, description, specs, images,
       sort_order, availability, condition, material, dimensions, brand, featured)
    VALUES
      (@name, @slug, @sku, @category, @price, @unit, @min_qty, @description, @specs, @images,
       @sort_order, @availability, @condition, @material, @dimensions, @brand, @featured)
  `).run({
    sort_order: 0, availability: 'in stock', condition: 'new',
    material: null, dimensions: null, brand: 'Arambhika Enablers', featured: 0,
    ...data,
  })
}
...
function updateProduct(id, data) {
  ...
  return db.prepare(`
    ...
    WHERE id=@id
  `).run({ brand: 'Arambhika Enablers', featured: 0, ...data, id })
}
